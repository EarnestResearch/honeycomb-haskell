...
spec = describe "Servant" $ do
  ...
  it "should blah" $
    getRequestInfo (Proxy :: Proxy Api2) call2
      `shouldBe` Just RequestInfo {pathSegments = [], pathParameters = []}
  it "should blah2" $
    getRequestInfo (Proxy :: Proxy Api3) call3
      `shouldBe` Just RequestInfo {pathSegments = ["hello"], pathParameters = []}
  it "should blah3" $
    getRequestInfo (Proxy :: Proxy Api4) call4
      `shouldBe` Just RequestInfo {pathSegments = ["users", ":user_id"], pathParameters = [("user_id", "100")]}
  it "should blah4" $ do
    getRequestInfo (Proxy :: Proxy Api5) call5a
      `shouldBe` Just RequestInfo {pathSegments = ["users", ":user_id"], pathParameters = [("user_id", "100")]}
    getRequestInfo (Proxy :: Proxy Api5) call5b
      `shouldBe` Just RequestInfo {pathSegments = ["info", "me"], pathParameters = []}
